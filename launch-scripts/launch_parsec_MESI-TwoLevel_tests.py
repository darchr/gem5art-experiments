...
gem5_binary = Artifact.registerArtifact(
    command = 'scons build/X86_MESI_Two_Level/gem5.opt',
    typ = 'gem5 binary',
    name = 'gem5',
    cwd = 'gem5/',
...
if __name__ == "__main__":
    ...
    for cpu in cpus:
        for num_cpu in num_cpus:
            for size in sizes:
                if cpu == 'timing' and (size != 'simsmall' or num_cpu == '8'):
                        continue
                for bm in benchmarks:
                    for boot in boot_types:
                        run = gem5Run.createFSRun(
                            'parsec_tests_multicore',    
                            'gem5/build/X86_MESI_Two_Level/gem5.opt',
                            'configs-parsec-tests-MESI-TwoLevel/run_parsec_MESI_TwoLevel.py',
                            f'''results/run_parsec_MESI_Two_Level/{bm}/{size}/{cpu}/{num_cpu}/{boot}''',
                            gem5_binary, gem5_repo, experiments_repo,
                            'linux-stable/vmlinux-4.19.83',
                            'disk-image/parsec/parsec-image/parsec',
                            linux_binary, disk_image,
                            cpu, bm, size, num_cpu, boot,
